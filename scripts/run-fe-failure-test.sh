...
OUT_DIR="${ROOT_DIR}/out/fe-failure-test"
...
while IFS= read -r file; do
  sources+=("${file}")
done < <(find "${ROOT_DIR}/src/test/java/com/dvrms/frontend" -name "*.java" | sort)

"${JAVAC_CMD}" -d "${OUT_DIR}" "${sources[@]}"

"${JAVA_CMD}" -cp "${OUT_DIR}" com.dvrms.frontend.FrontEndFailureHandlingTest
